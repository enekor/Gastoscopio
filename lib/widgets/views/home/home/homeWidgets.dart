...
Widget MinimalistGreetingText() {
  return Text(
    '${GetGreeting()} ${Values().cuentaRet.value!.Nombre}!',
    style: const TextStyle(fontSize: 25),
...
Widget LastInteractionsPart({required BuildContext context}) {
  ...
  return Card(
      color: GetColor(ColorTypes.background, context)
          .withOpacity(!Values().mostrarFondoDinamico.value ? 1 : 0.5),
...
Widget MinimalistTotalPart() {
  return Obx(() => Text(
      '${Values().mes.value} ${Values().cuentaRet.value!.GetTotal(Values().anno.value, Values().mes.value).toStringAsFixed(2)}${Values().moneda.value}',
      textAlign: TextAlign.center,
...
Widget InfoBottomNavigationBar(
  ...
  return NavigationBar(
    ...
    backgroundColor: GetColor(ColorTypes.background, context).withOpacity(0.7),
    destinations: [
